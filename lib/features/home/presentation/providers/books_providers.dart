...
final booksProviders = NotifierProvider<_BooksNotifier, BooksState>(_BooksNotifier.new,
);
...
final class _BooksNotifier extends Notifier<BooksState> {
  ...
  Future<void>fetchBookCategories() async {
   ...
   failures[0] = result.getLeft();
    ...
    state = state.copyWith(
        ...
        categoryIds: result.getOrElse((l) => []).map((category) => category.id).toList(),
        failures: failures,
        isLoading: false,
      );
      print(state.bookCategories.length);
  ...
  Future<void> fetchBooksBydALlId() async {
          ...
          Map<int, Option<Failure>> failures=Map.from(state.failures);
          Future (() => setLoading( true));



    for (final categoryId in state.categoryIds){
      final result = await ref.read(homeRepositoryProvider).getBooksByCategory(categoryId);
  ...
  Future<void> fetchBookImage() async {
    ...
    for (final categoryId in state.categoryIds){
      for (final book in state.books[categoryId]!){
        final result = await ref.read(homeRepositoryProvider).getBookImage(book.cover);
        tempBooks[categoryId] = tempBooks[categoryId]!.map((b) => b.id == book.id ? b.copyWith(image: result.getOrElse((l) => '')) : b).toList();
...
void categoryFilterChanged(int categoryId) {
  ...
  if (categoryId == -1) {
    ...
    categoryIds = state.bookCategories.map((category) => category.id).toList();
  } else {
...
void searchFilterChanged(String search)  {
      ...
      if(search.isEmpty) {
        state.selectedCategoryId==-1?categoryIds=state.bookCategories.map((category) => category.id).toList():categoryIds=[state.selectedCategoryId];
        state = state.copyWith(filteredBooks: state.books, searchQuery: search, categoryIds: categoryIds);
        return;
      }
      ...
      var filteredList = bookList.where((book) => book.name.toLowerCase().contains(search.toLowerCase()) || book.author.toLowerCase().contains(search.toLowerCase())).toList();
    if(filteredList.isEmpty) {
      categoryIds.remove(key);//tel bir id yoksa allda arama yaptığımız için arayüzde rahat göstermek için boş idleri siliyorum 
    }
    filteredBooks[key] = filteredList;
      ...
      else
      {
        ...
        List<Book> filteredList = state.books[categoryIds[0]]!.where((book) => book.name.toLowerCase().contains(search.toLowerCase()) || book.author.toLowerCase().contains(search.toLowerCase())).toList();
      ...
      state = state.copyWith(filteredBooks: filteredBooks, searchQuery: search, categoryIds: categoryIds,);
    }
